...
public class CreateAccount {
    ...
    public CreateAccount(String firstName, String lastName, String email, String pw, InputStream url, String Extension, Boolean isManager, ProgressBar bar, AlertDialog alert){
        Log.i("CreateAccount","in create account");

        String picUrl=AWSLink(email,Extension);
        String hashedPw = BCrypt.withDefaults().hashToString(12, pw.toCharArray());
        Account a = new StudentAccount(firstName,lastName,email,hashedPw,picUrl,isManager);
        FirebaseTest.checkEmailExists(email,bar,alert,a,url,Extension);

    }

    public CreateAccount(String firstName, String lastName, String email, String pw,InputStream url,String Extension ,Boolean isManager,Long id, String major,ProgressBar bar,AlertDialog alert){
        Log.i("CreateAccount","in create account");
